...
void test_interpolation() {
    ...
    assert(g_nearly_equal(res.x, 0.5f) && g_nearly_equal(res.y, 0.5f) && g_nearly_equal(res.z, 0.0f) && g_nearly_equal(res.w, 0.0f));

    // SLERP 50% (should be at 45 deg on the arc)
    vector_slerp(&start, &end, 0.5f, &res);
    // 0.7071 is cos(45)
    assert(fabsf(res.x - 0.7071f) < TRIG_EPSILON);
    assert(fabsf(res.y - 0.7071f) < TRIG_EPSILON);
...
void test_orthogonality_and_cross() {
    ...
    assert(fabsf(dot_a - 0.0f) < TRIG_EPSILON && "Cross product not perp to A!");
    assert(fabsf(dot_b - 0.0f) < TRIG_EPSILON && "Cross product not perp to B!");
